...
public class ProductDao {
	...
	public void insert(Connection conn, Product product) 
		...
		try {
			...
			("insert into product (name,maker,price,discount,origin,thumbnailimg,detailimg,fav,category)"
					+ " values(?,?,?,?,?,?,?,?,?)");
	...
	public void update(Connection conn, Product product) 
		...
		try {
			pstmt = conn.prepareStatement
			("update product set name=?,maker=?,price=?,discount=?,origin=?,thumbnailimg=?,detailimg=?,fav=?,category=? where productId=?");
			pstmt.setString(1, product.getName());
			pstmt.setString(2, product.getMaker());
			pstmt.setInt(3, product.getPrice());
			pstmt.setInt(4, product.getDiscount());
...
*/
	// final?? ?àò?†ï?ù¥ ?ïà?êú?ã§?äî ?Ç§?õå?ìú
	// ?éò?ù¥Ïß? ?ãπ 5Í∞?
	...
	public ProductListView getProductList(Connection conn, int pageNumber)
		...
		try {
			if (productTotalCount > 0) {
				...
				productList = selectListLimit(conn, firstRow, endRow);
			} else {
				currentPageNumber = 0;
